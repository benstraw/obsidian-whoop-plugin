...
export function renderWeekly(stats: WeekStats): string {
  ...
  const navPrev = `[[Health/WHOOP/${pwy}/${pw}|← Prev Week]]`;
  const navNext = `[[Health/WHOOP/${nwy}/${nw}|Next Week →]]`;
  ...
  const breakdownRows = stats.days
    .map((d) => {
      ...
      const hrv = d.recovery
        ...
        : "—";
      const strain = d.cycle ? fmt1(d.cycle.score.strain) : "—";
...
# WHOOP Weekly Summary — ${stats.weekStart} → ${stats.weekEnd}
...
| Color | Days |
|-------|------|
| 🟢 Green (67–100%) | ${stats.greenDays} |
| 🟡 Yellow (34–66%) | ${stats.yellowDays} |
| 🔴 Red (0–33%) | ${stats.redDays} |
